...
obsync holds a credential that can write to your repository, unattended. This
page states the smallest one that works for each remote, what obsync does with
it, and how you find out you got it wrong.

**The secret is always a file, never a variable.** `OBSYNC_TOKEN_FILE` names a
path obsync can read, and obsync re-reads it every single time git asks for a
credential — so rotating a token is a write to that file, with no restart and no
redeploy. There is no `OBSYNC_TOKEN`.

---

## What each remote form needs

The repo forms obsync accepts, and the credential each one takes. The forms
themselves are the declared surface, and they are stated on
...
Give obsync the least a working sync needs, which on every remote here is: read
and write the contents of **one** repository, and nothing else.

| Remote | Credential | Scope | `OBSYNC_USERNAME` |
|---|---|---|---|
| **GitHub** | Fine-grained PAT, scoped to the one repository | `Contents: Read and write`, plus the `Metadata: Read` that GitHub adds for you and will not let you remove | Any non-empty value; obsync's default `obsync` is fine |
| **GitHub** | Classic PAT — the fallback, not the recommendation | `repo`, which grants vastly more than this, across every repository you can reach | As above |
...
**GitHub's own page is the instruction**, and it is deliberately not reproduced
here as a click path: a walkthrough of somebody else's UI is the best reading
experience for a month and wrong for years afterwards.
...
time you look, and it does that without a startup probe:

- The first push obsync **attempts** and does not land is an **ERROR
  immediately** and **unhealthy at once** — not backed off quietly, because
  nobody has ever seen this deployment work.
- Once a push has succeeded, the same failure is ordinary backoff and an hourly
  repeat instead.
...
the second is a credential problem.

---

## SSH

**SSH needs no knobs.** The URL scheme is what chooses the credential path, and
obsync stores and manages neither the key nor the `known_hosts` — they arrive as
ordinary mounts, and obsync never reads or writes either itself.
...
`./secrets/ssh` holds the two files ssh expects to find in a `.ssh` directory:

- **The private key**, mode `0600` and owned by the UID in the compose file's
  `user:` line. ssh refuses to use a private key that is group- or
  world-readable, and says so rather than falling back to anything.
- **`known_hosts`**, because obsync runs with no terminal at all and can never
  answer *"are you sure you want to continue connecting?"*. Produce it once,
  and check the fingerprint against what your host publishes:

  ```bash
  ssh-keyscan github.com > ./secrets/ssh/known_hosts
  ```

> **Load-bearing documentation** (§8, [#12](../../issues/12)).
> **The second mount is the one nobody expects, and leaving it out is silent.**
> The image bakes no `/etc/passwd` entry for the UID Docker's `user:` line names
> — that is the whole point of it — and **ssh expands `~` out of that entry
> rather than out of `HOME`**. Measured in obsync's own image (alpine 3.23,
> OpenSSH 10.2): with no entry at all ssh exits before it reads any
> configuration, `No user exists for uid 1000`; with an entry whose home is
> somewhere else, it looks for the key somewhere else. Either way git reports a
> remote it could not read, obsync tiers that as an unreachable remote, and an
> unreachable remote is **healthy and quiet for 24 hours** — over a vault that
> has never once been backed up. One line for this UID is the whole file, and
> its home field must be the image's `HOME`. Never cut this.
>
> ```
...
> from its private git config, which is why no root entrypoint and no
> `PUID`/`PGID` knob exist. This mount is ssh's requirement, not obsync's.

An SSH remote needs **no** `OBSYNC_TOKEN_FILE` and no `OBSYNC_USERNAME`: the
user is in the URL, and `git@host:owner/vault.git` and
`ssh://git@host/owner/vault.git` are the same repo as far as obsync's check on
`origin` is concerned.
...
rather than a mistake to be refused.

It is the weakest of the warnings in this documentation set, because the startup
WARN fires whether or not anybody read this page. If the remote is reachable
from anything you do not control, use `https://` or `ssh://`.
...
- **It never reaches a URL.** A token in the remote URL leaks into
  `git remote -v`, into every log line, and into the check obsync makes that
  `origin` is still the remote it was given.
- **It never reaches an argv**, so DEBUG logging — which prints every git
  invocation with its full argv — is safe to turn on.
- **It never reaches a file obsync wrote**, a socket, or a daemon. obsync is its
  own git credential helper: git asks, obsync reads your file, obsync prints it
  to git, and that is the whole mechanism. There is no cache, because a cache
  exists specifically to *not* re-read.
...
obsync says so and exits 1, because it is decidable from the environment block
alone. The same file becoming unreadable **later** is not — that is a credential
being rotated, obsync keeps committing locally, and the next run after the file
comes back publishes everything.
